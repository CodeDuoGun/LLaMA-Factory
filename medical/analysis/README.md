...
- 疾病、病情和内服处方的关系；
- 按规范化后的 `diagnosis_illness` 统计疾病核心用药、病种内使用率及相对全体处方的 Lift；
...
1. 疾病使用 `diagnosis_illness` 规范化值分组：多项诊断会统一分隔符为顿号、去重并稳定排序；不进行器官、病理类型或同义词类别合并。
...
5. 患者时间轴的“本次处方明细”展示当前节点的全部处方、各处方 `usage_type`，以及药物名称、剂量和单位。
6. 单次问诊有多张内服处方时，时间轴会全部展示；前后调方比较仍选择药味数最多的完整主方，避免直接使用 `ps[0]`。
